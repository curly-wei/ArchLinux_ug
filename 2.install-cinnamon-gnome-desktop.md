...
<1> Proprietary driver (recommand, >=GTX-700s)
...
```
### <1.2> Add pacman hook to compile module on kernel upgrades
...
### <1.3> Blacklist nouveau driver

```
sudo bash -c "echo blacklist nouveau > /etc/modprobe.d/blacklist-nvidia-nouveau.conf"
```

### <1.4> Add graphics card configuration in Xorg server
...
### <1.5> Load nvidia modules on boot – update firmware
...
<2> open source driver (bad performance, not recommand)
